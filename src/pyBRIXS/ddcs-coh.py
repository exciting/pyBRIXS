...
def main(cfg_path="input-ddcs.cfg"):
    ...
    # --- classic ---
    if any(r.spectrum is not None for r in rixs_list):
        spectrum_list = [r.spectrum for r in rixs_list if r.spectrum is not None]
        getdata = GetData(ecore_1, eloss, ecoreindex_1, spectrum_list)
        with open(output_file, 'w') as f:
            getdata.write_ddcs(f)

    # --- coherent ---
    if any(r.spectrum_coh is not None for r in rixs_list):
        spectrum_list = [r.spectrum_coh for r in rixs_list if r.spectrum_coh is not None]
        getdata = GetData(ecore_1, eloss, ecoreindex_1, spectrum_list)
        with open(output_file + "_coherent", 'w') as f:
            getdata.write_ddcs(f)

    # --- incoherent ---
    if any(r.spectrum_incoh is not None for r in rixs_list):
        spectrum_list = [r.spectrum_incoh for r in rixs_list if r.spectrum_incoh is not None]
        getdata = GetData(ecore_1, eloss, ecoreindex_1, spectrum_list)
        with open(output_file + "_incoherent", 'w') as f:
            getdata.write_ddcs(f)
